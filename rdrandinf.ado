...
program define rdrandinf, rclass sortpreserve
	...
	local Y "`1'"
	local r "`2'"
	qui gen `tr' = (`r' >= `cutoff') if `touse'
	qui gen `runvar' = `r' - `cutoff'
...
********************************************************************************
	...
	if "`wl'"!="" & "`wr'"!=""{
		...
		if "`covariates'"!=""{
			...
			if "`quietly'"==""{
				rdwinselect `r' `covariates' if `touse', c(`cutoff') `obsmin_opt' `obsstep_opt' `wmin_opt' `wstep_opt' /*
					*/ `wobs_opt' `nwindows_opt' `rdwstat_opt' `approximate_opt' reps(`rdwreps') `level_opt' `plot_opt' `graph_opt'
			}
			else {
				qui rdwinselect `r' `covariates' if `touse', c(`cutoff') `obsmin_opt' `obsstep_opt' `wmin_opt' `wstep_opt' /*
			...
			local wl = `cutoff' - r(rec_length)
			local wr = `cutoff' + r(rec_length)
	...
	if "`fuzzy'"==""{
		...
		if "`statistic'"!=""{
			...
			if "`fuzzy_stat'"=="ar"|"`fuzzy_stat'"==""{
				...
				local fuzzy_cond "endogtr(`fuzzy_treat')"
				if "`ci'"!="" & `p'>0{
					di as error "ci not allowed with tsls and p>0"
					exit 198
				}
			}
			else {
		...
		if "`fuzzy'"!=""{
			...
			if "`fuzzy_stat'"=="ar"|"`fuzzy_stat'"==""{
				...
				if `p'==0{
					...
					local ci_lb = `obs_stat' - 1.96*_b[`fuzzy_treat']/_se[`fuzzy_treat']
					local ci_ub = `obs_stat' + 1.96*_b[`fuzzy_treat']/_se[`fuzzy_treat']
				}
				else {
					forvalues k=1/`p'{
						capture gen _runpoly_all`k'=`runvar'^`k'
...
********************************************************************************
	...
	if "`ci'"!=""{
		if "`fuzzy_stat'"!="tsls"{	
			...
			qui rdsensitivity `Y' `runvar', wlist(`wr') `stat_opt_ci' `fuzzy_cond_ci' `tlist_opt' ci(`wr' `ci_level') nodraw reps(`reps') 
			local ci_lb = r(ci_lb)
			local ci_ub = r(ci_ub)
			di "Confidence interval obtained."
		}

		di as text _newline "Confidence interval for w = `wr'"
