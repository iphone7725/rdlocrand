...
{p 4 8}{cmd:wl(}{it:#}{cmd:)} specifies the left limit of the window. Default is the minimum of the running variable. {p_end}

{p 4 8}{cmd:wr(}{it:#}{cmd:)} specifies the right limit of the window. Default is the maximum of the running variable. {p_end}
...
{p 4 8}{cmd:ci(}{it:level [tlist]}{cmd:)} calculates a confidence interval for the treatment effect by test inversion, where {it: level} specifies the level 
of the confidence interval and {it: tlist} indicates the grid of treatment effects to be evaluated.
This option uses {cmd:rdsensitivity} to calculate the confidence interval;  type {cmd: help rdsensitivity} for details.{p_end}

{p 4 8}{cmd:{opt interf:ci}(}{it:#}{cmd:)} sets the level for Rosenbaum's confidence interval under arbitrary interference between units. See {help rdrbounds:rdrbounds} for details.{p_end}
...
{p 4 8}{cmd:{opt be:rnoulli}(}{it:varname}{cmd:)} specifies that the randomization mechanism is Bernoulli trials (instead of fixed margins randomization). The values of the probability of treatment for each unit must be provided in the variable {cmd: varname}.{p_end}
...
{p 4 8}NOTE: When the window around the cutoff is not specified, {cmd:rdrandinf} can select the window automatically using the companion command {help rdwinselect:rdwinselect}. The following options are available:{p_end}

{p 8 12}{cmd:{opt cov:ariates}(}{it:varlist}{cmd:)} specifies the covariates employed by the companion command {help rdwinselect:rdwinselect}.{p_end}

{p 8 12}{cmd:obsmin(}{it:#}{cmd:)} specifies the minimum number of observations above and below the cutoff in the smallest window employed by the companion command {help rdwinselect:rdwinselect}. Default is {cmd:obsmin(10)}.{p_end}

{p 8 12}{cmd:obsstep(}{it:#}{cmd:)} specifies the minimum number of observations to be added on each side of the cutoff for the sequence of nested windows constructed by the companion command {help rdwinselect:rdwinselect}. Default is {cmd:obsstep(2)}.{p_end}

{p 8 12}{cmd:wmin(}{it:#}{cmd:)} specifies the smallest window to be used (if {cmd: minobs(}{it:#}{cmd:)} is not specified) by the companion command {help rdwinselect:rdwinselect}.
Specifying both {cmd:wmin(}{it:#}{cmd:)} and {cmd:obsmin(}{it:#}{cmd:)} returns an error.{p_end}

{p 8 12}{cmd:wstep(}{it:#}{cmd:)} specifies the increment in window length (if {cmd:obsstep(}{it:#}{cmd:)} is not specified) by the companion command {help rdwinselect:rdwinselect}.
Specifying both {cmd:obsstep(}{it:#}{cmd:)} and {cmd:wstep(}{it:#}{cmd:)} returns an error.{p_end}

{p 8 12}{cmd:{opt nw:indows}(}{it:#}{cmd:)} specifies the number of windows to be used by the companion command {help rdwinselect:rdwinselect}. Default is {cmd:nwindows(10)}.{p_end}

{p 8 12}{cmd:rdwstat(}{it:#}{cmd:)} specifies the statistic to be used by the companion command {help rdwinselect:rdwinselect} (see help file for options). Default option is {cmd:rdwstat(ttest)}.{p_end}

{p 8 12}{cmd:{opt approx:imate}} forces the companion command {help rdwinselect:rdwinselect} to conduct the covariate balance tests using a large-sample approximation instead of finite-sample exact randomization inference methods.{p_end}

{p 8 12}{cmd:rdwreps(}{it:#}{cmd:)} specifies the number of replications to be used by the companion command {help rdwinselect:rdwinselect}. Default is {cmd:rdwreps(1000)}.{p_end}

{p 8 12}{cmd:level(}{it:#}{cmd:)} specifies the minimum accepted value of the p-value from the covariate balance tests to be used by the companion command {help rdwinselect:rdwinselect}. Default is {cmd:level(.15)}.{p_end}

{p 8 12}{cmd:plot} draws a scatter plot of the minimum p-value from the covariate balance test against window length implemented by the companion command {help rdwinselect:rdwinselect}.{p_end}

{p 8 12}{cmd:graph_options(}{it:graphopts}{cmd:)} graph options for plot generated by the companion command {help rdwinselect:rdwinselect}.{p_end}

{p 8 12}{cmd:{opt qui:etly}} supress output from  the companion command {help rdwinselect:rdwinselect}.{p_end}
...
{synopt:{cmd:r(wl)}} left limit of window used{p_end}
{synopt:{cmd:r(wr)}} right limit of window used{p_end}
